...
**FIGURE 1 - OVERVIEW OF THE SSRG PIPELINE** I. Genomes can be downloaded automatically from NCBI using provided scripts and custom or NCBI-generated lists. II. [SSRG.pl](https://github.com/PombertLab/SNPs/blob/master/SSRG/SSRG.pl) generates FASTQ datasets from FASTA files at user-specified read lengths and desired sequencing depth. Note that this approach should be used only for haploid genomes. https://github.com/PombertLab/SNPs/blob/master/SSRG/SSRG.pl is especially useful to compare genomes in databases for which sequencing reads are unavailable. III. [get_SNPs.pl](https://github.com/PombertLab/SNPs/blob/master/SSRG/get_SNPs.pl) maps FASTQ files against references genomes using [BWA](http://bio-bwa.sourceforge.net/), [Bowtie2](http://bowtie-bio.sourceforge.net/bowtie2/index.shtml), [HISAT2](https://ccb.jhu.edu/software/hisat2/index.shtml), [Minimap2](https://github.com/lh3/minimap2) or [NGMLR](https://github.com/philres/ngmlr) as specified by the user. SNPs and indels (optional) are then calculated with [Samtools](http://www.htslib.org/) + [VarScan2](http://dkoboldt.github.io/varscan/), [BCFtools](http://samtools.github.io/bcftools/), or [FreeBayes](https://github.com/ekg/freebayes)]. IV. [sort_stats.pl](https://github.com/PombertLab/SNPs/blob/master/SSRG/sort_stats.pl) generates a tab-delimited table of SNP metrics. V. [run_Mash.pl](https://github.com/PombertLab/SNPs/blob/master/MASH/run_Mash.pl) can estimate genetic distances using the MinHash Reduction technique, as implemented in [Mash](https://github.com/marbl/Mash). VI. [MashToDistanceMatrix.pl](https://github.com/PombertLab/SNPs/blob/master/MASH/MashToDistanceMatrix.pl) converts the output of Mash to distance matrices. VII. [MashR_plotter.pl](https://github.com/PombertLab/SNPs/blob/master/MASH/MashR_plotter.pl) can A) clusters operational taxonomic units (OTUs) according to their estimated genetic distances, using R and either MDS or t-SNE algorithms, B) plot Neighbor-joining or UPGMA trees from Mash distances, C) generate clustered heatmaps from these distances.
