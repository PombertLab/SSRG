...
This can be done by entering a query in the search box, then by clicking on the lineage link (*e.g.* ***Prokaryotes***) next to ***Overview***. This links redirects to the corresponding genomes in NCBI. 
...
1. Download FASTA and GenBank files from a small list of 3 *Streptococcus pneumoniae* genomes with NCBI with [queryNCBI.pl](https://github.com/PombertLab/SSRG/blob/master/Tools/NCBI/queryNCBI.pl). This list was generated by the NCBI genome database (https://www.ncbi.nlm.nih.gov/genome/browse/#!/prokaryotes/176/). To download the genomes, type:
```bash
queryNCBI.pl -l Examples/S_pneumoniae_3.csv -o DATASETS -fa -gb
```

2. Generate FASTQ datasets from the downloaded genomes with [SSRG.pl](https://github.com/PombertLab/SSRG/blob/master/SSRG.pl).
```bash
