...
#########################################################################
### Building database of sequences
#########################################################################
...
while (my $vcf = shift@vcf){
    ...
    for (0..$#feat_headers-1){
		print FEAT "$feat_headers[$_]\t";
	}
    ...
    for (0..$#inter_headers-1){
		print INTER "$inter_headers[$_]\t";
	}
    ...
    while (my $line = <VCF>){
        ...
        if ($line =~ /^#/){
            ...
            if (exists $loci{$contig}{$position}){
				...
				print INTER "$contig\t$vcf\tintergenic\t$position\t$ref\t$alt\t$freq\t$evalue\n"
			}
        }
    }
}
