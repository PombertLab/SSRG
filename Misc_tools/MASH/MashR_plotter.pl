...
# Requires R, R-devel
# Requires the Rtsne, igraph, lattice, ape and phangorn R packages
...
--type (-t)		Plot type: cluster, tree, heatmap [Default: cluster] ## heatmap = alpha
...
if ($type eq 'cluster'){
	...
	Rhm();
	if ($format eq 'pdf'){pdf();}
	my $num = `grep -c -v \'OTU\' $input`; chomp $num; $num++;
	print OUT 'library(lattice)'."\n";
	print OUT 'm=as.matrix(distance_matrix[1:'."$num".', 2:'."$num".'])'."\n";
	print OUT 'levelplot(m[1:ncol(m),ncol(m):1])'."\n";
	close IN;
	close OUT;
}
